...
export const tabsChess = pgTable('tabs_chess', {
  tabId: uuid('tab_id').primaryKey().defaultRandom(),
  title: text('title').notNull(),
  slug: text('slug').notNull(),
  track:text('track').notNull().default('Chess'),
...
(t)=>({
    ...
    eventIdIdx: index('tabs_chess_eventId_idx').on(t.eventId),
})
);

export const tabChessRelations= relations(tabsChess,({one})=>({
    event: one(events,{
        fields:[tabsChess.eventId],
