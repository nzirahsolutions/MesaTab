...
export default function LogIn() {
  ...
  async function google(c){
    setLoading(true);
      try {
        const res=await axios.post(`${currentServer}/user/glogin`,{gtoken: c});
        ...
        setErrorMessage(err?.response?.data?.message || "Google Log in failed. Check your connection and try again.");       
      }
  }
  ...
  return (
    ...
    <form action="">
      ...
      <GoogleLogin onSuccess={(credentialResponse)=>google(credentialResponse.credential)} 
      onError={()=>{
        setLoading(false);
        setError(true);
        localStorage.removeItem('token');
